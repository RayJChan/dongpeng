...
public class ImportExcelUtils{
    ...
    public <E> List<E> getDataList(Class<E> cls, int... groups) throws InstantiationException, IllegalAccessException{
        ...
        List<Field> fs=ReflectionsUtils.getClassGenricFields(cls);
        for (Field f : fs){
        ...
        List<Method> ms=ReflectionsUtils.getClassGenricMethods(cls);
        for (Method m : ms){
        ...
        // Get excel data
        Row headRow=this.getRow(headerNum);//头部标题行
        List<E> dataList = Lists.newArrayList();
        //log.warn("this.getLastCellNum() = "+this.getLastCellNum());
        for (int i = this.getDataRowNum(); i < this.getLastDataRowNum(); i++) {
            ...
            for (Object[] os : annotationList){
                ...
                Object val=null;
                for(int c=0;c<this.getLastCellNum();c++){
                    String title=this.getCellValue(headRow, c).toString();
                    if(StringPlusUtils.isNotBlank(title) && title.equals(ef.title())){
                        //log.warn("title："+title);
                        val=this.getCellValue(row, c);
                        break;
                    }
                }
                if (val != null){
     ...
     * @throws Exception
     */
    public List<Map<Integer, String>> getDataList() throws Exception {
            ...
            if (row != null) {
                ...
                for (int colNum = 0; colNum < row.getLastCellNum(); colNum++) {
                    if (row.getCell(colNum) != null) {
                        rowMap.put(colNum, row.getCell(colNum).toString());
                    } else {
                        rowMap.put(colNum, "");
                    }
                }
