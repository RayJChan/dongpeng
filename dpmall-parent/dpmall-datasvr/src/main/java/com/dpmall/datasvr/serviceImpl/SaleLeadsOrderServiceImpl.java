...
//----------------------------------正式-------------------------------
	@Override
	public List<SaleLeadsOrderModel> getList4Agency(String agencyId, String listStatus, Integer pageNum, Integer pageSize,String search, String orderId, String clientName, String clientTel, String staRevisitTime,String endRevisitTime, String statusSearch) {
		List<SaleLeadsOrderModel> out = new ArrayList<SaleLeadsOrderModel>();

		//列表数量
		String totalCount = salesLeadsOrderDao.getListCount("Y",agencyId, listStatus,search,orderId,clientName,clientTel,staRevisitTime,endRevisitTime,SaleLeadsOrderStatus.CHSTOENGLISH(statusSearch));
		...
		//经销商
		List<SalesLeadsOrderEntity> in = salesLeadsOrderDao.getList4AgencyOrStore("Y",agencyId, listStatus,pageNum,pageSize,search,orderId,clientName,clientTel,staRevisitTime,endRevisitTime,SaleLeadsOrderStatus.CHSTOENGLISH(statusSearch));
	 ...
	 */
	@Override
	public List<SaleLeadsOrderModel> getList4AgencyOfUnHandle(String agencyId, Integer pageNum, Integer pageSize, String search) {
		List<SaleLeadsOrderModel> out = new ArrayList<SaleLeadsOrderModel>();

		//列表数量
		int totalCount = salesLeadsOrderDao.getCountUnWaitting(agencyId);
		if("0".equals(totalCount)){
			return out;
		}

		//数据列表
		List<SalesLeadsOrderEntity> in = salesLeadsOrderDao.getList4AgencyOf(agencyId,pageNum,pageSize,search);
		...
		out.get(0).setTotalCount(String.valueOf(totalCount));//列表数量，只有第一个元素有
		return out;
	}


	@Override
	public List<SaleLeadsOrderModel> getList4Store(String storeId, String listStatus, Integer pageNum, Integer pageSize,String search, String orderId, String clientName, String clientTel, String staRevisitTime,String endRevisitTime, String statusSearch) {
		List<SaleLeadsOrderModel> out = new ArrayList<SaleLeadsOrderModel>();

		//列表数量
		String totalCount = salesLeadsOrderDao.getListCount("N",storeId, listStatus,search,orderId,clientName,clientTel,staRevisitTime,endRevisitTime,SaleLeadsOrderStatus.CHSTOENGLISH(statusSearch));
		if("0".equals(totalCount)){
			return out;
		}
		//经销商
		List<SalesLeadsOrderEntity> in = salesLeadsOrderDao.getList4AgencyOrStore("N",storeId, listStatus,pageNum,pageSize,search,orderId,clientName,clientTel,staRevisitTime,endRevisitTime,SaleLeadsOrderStatus.CHSTOENGLISH(statusSearch));
        if (CollectionUtils.isEmpty(in)){
            return out;
        }
		if ("1".equals(listStatus)) {
...
//			Po.setOrderGuide(operatorBy);//接单员
			...
			//更新留资单表
			result = salesLeadsOrderDao.edit(Po);
...
//			}
	...
	public SaleLeadsOrderDetailsModel getDetails(String saleLeadsOrderId,String operatorById) {
		...
		if(saleLeadsOrderId==null || detailsIn ==null){
			return result;
		}
		CglibBeanCopierUtils.copyProperties(detailsIn,result);//复制对象
		//待接单
...
//				builder.delete(0,builder.length());//清空builder
	...
	public int updateCustomerInfo(SaleLeadsOrderModel in,String operatorBy) {
		...
		SalesLeadsOrderPo Po = new SalesLeadsOrderPo();
		CglibBeanCopierUtils.copyProperties(in,Po);
	...
	@Override
	@Transactional
	public int updateOrderProgress(SaleLeadOrderModelIn form) {
		String statusName = form.statusName;
		Long saleLeadsOrderId = form.saleLeadsOrderId;
		String remark = form.remark;
		String operatorBy = form.operatorBy;
		String failType = form.failTypeId;
		...
		SalesLeadsOrderEntity details = salesLeadsOrderDao.getDetails(saleLeadsOrderId.toString());
		if (details == null) {
			return 0;
		}

		String lastStatus =details.getStatus();
		...
		po.setOrderEvaluation(form.orderEvaluation);//订单评价
		po.setIsLoss(form.isLoss);//是否流失
		po.setRevisitTime(form.revisitTime);//回访时间
...
//		}
	...
	private int update (SalOrderInfoModelIn modelIn,SalesLeadsOrderPo po){
		...
		//更新备注信息 (需要判断门店or经销商)
		SalesLeadsOrderEntity details = salesLeadsOrderDao.getDetails(modelIn.getSaleLeadsOrderId().toString());
		String isAgency = details.getIsAgency();
		...
		if (modelIn.getSaleLeadsOrderGoods() !=null){
			...
			for (SalOrderGoodsModelIn goodsModelIn : modelIn.getSaleLeadsOrderGoods()) {
				...
				//根据产品编码查询商品表信息
				String shangpId = salesLeadsOrderItemDao.getShangpIdByCode(goodsModelIn.getProductCode()) ;
				if (StringUtils.isNotEmpty(shangpId)){
					itemsPo.setShangpId(shangpId);
				}
	...
	@Override
	public SaleLeadsOrderModel getStatusInfoById(String salesLeadsOrderId) {
		SaleLeadsOrderModel out  = new SaleLeadsOrderModel();
		SalesLeadsOrderEntity entity = salesLeadsOrderDao.getStatusInfoById(salesLeadsOrderId);
		CglibBeanCopierUtils.copyProperties(entity,out);
		return out;
	}
}
