...
public class FirstPageServiceImpl implements IFirstPageService {
	...
	public FirstPageModel getFirstPageOfAgency(String code) {
		...
		//5、实物订单---------------------------------------------------------------------------
		BigDecimal orderWaitAcceptCount = firstPageDao.orderCountOfAgency(agencyIds, bigList, startdate, enddate, "1");//待接单数数量
		orderWaitAcceptCount = orderWaitAcceptCount == null ? BigDecimal.ZERO : orderWaitAcceptCount;

		BigDecimal orderFollowingCount = firstPageDao.orderCountOfAgency(agencyIds, bigList, startdate, enddate, "2");// 跟进中数量
		orderFollowingCount = orderFollowingCount == null ? BigDecimal.ZERO : orderFollowingCount;

		BigDecimal orderWaifForRefundCount = firstPageDao.orderCountOfAgency(agencyIds, bigList, startdate, enddate, "3");// 待返款
		orderWaifForRefundCount = orderWaifForRefundCount == null ? BigDecimal.ZERO : orderWaifForRefundCount;

		BigDecimal orderFinishedCount = firstPageDao.orderCountOfAgency(agencyIds, bigList, startdate, enddate, "4");// 已完成
		orderFinishedCount = orderFinishedCount == null ? BigDecimal.ZERO : orderFinishedCount;
		
		BigDecimal lastOrderFinishedCount = firstPageDao.orderCountOfAgency(agencyIds,bigList, lastStartdate, lastEnddate,"4");// 上个月已完成数量
		...
		//为model赋值
		 ...
		 //6、天猫特权订金-----------------------------------------------------------------------
		//待接单数数量
		 BigDecimal tmallPrePayWaitAcceptCount = firstPageDao.tmallPrepayCountOfAgency(agencyIds, bigList, startdate, enddate, "1");
		tmallPrePayWaitAcceptCount = tmallPrePayWaitAcceptCount == null ? BigDecimal.ZERO : tmallPrePayWaitAcceptCount;
		//已接单数数量
		 BigDecimal tmallPrePayFinishedCount = firstPageDao.tmallPrepayCountOfAgency(agencyIds, bigList, startdate, enddate, "2");
		tmallPrePayFinishedCount = tmallPrePayFinishedCount == null ? BigDecimal.ZERO : tmallPrePayFinishedCount;
		// 上个月已接单数数量
		 BigDecimal lastTPrePayFinishedCount = firstPageDao.tmallPrepayCountOfAgency(agencyIds,bigList, lastStartdate, lastEnddate,"2");
...
//暂时
	...
	public FirstPageModel getFirstPageOfStore( String storeId) {
		...
		//5、实物订单---------------------------------------------------------------------------
		BigDecimal orderWaitAcceptCount = firstPageDao.orderCountOfStore(startdate, enddate, "1", storeId);//待接单数数量
		orderWaitAcceptCount = orderWaitAcceptCount == null ? BigDecimal.ZERO : orderWaitAcceptCount;

		BigDecimal orderFollowingCount = firstPageDao.orderCountOfStore(startdate, enddate, "2", storeId);// 跟进中数量
		orderFollowingCount = orderFollowingCount == null ? BigDecimal.ZERO : orderFollowingCount;

		BigDecimal orderFinishedCount = firstPageDao.orderCountOfStore(startdate, enddate, "3", storeId);// 已完成
		orderFinishedCount = orderFinishedCount == null ? BigDecimal.ZERO : orderFinishedCount;
		
		BigDecimal lastOrderFinishedCount = firstPageDao.orderCountOfStore(lastStartdate, lastEnddate, "3", storeId);// 上个月已完成数量
		lastOrderFinishedCount = lastOrderFinishedCount == null ? BigDecimal.ZERO : lastOrderFinishedCount;
		...
		//为model赋值
		 ...
		 //6、天猫特权订金-----------------------------------------------------------------------
		//待接单数数量
		 BigDecimal tmallPrePayWaitAcceptCount = firstPageDao.tmallPrepayCountOfStore(startdate, enddate, "1", storeId);
		tmallPrePayWaitAcceptCount = tmallPrePayWaitAcceptCount == null ? BigDecimal.ZERO : tmallPrePayWaitAcceptCount;
		//已接单数数量
		 BigDecimal tmallPrePayFinishedCount = firstPageDao.tmallPrepayCountOfStore(startdate, enddate, "2", storeId);
		tmallPrePayFinishedCount = tmallPrePayFinishedCount == null ? BigDecimal.ZERO : tmallPrePayFinishedCount;
		// 上个月已完成数量
		 BigDecimal lastTPrePayFinishedCount = firstPageDao.tmallPrepayCountOfStore(lastStartdate, lastEnddate, "2", storeId);
		lastTPrePayFinishedCount = lastTPrePayFinishedCount == null ? BigDecimal.ZERO : lastTPrePayFinishedCount;
		//比较上个月数据
		 String tmallPrePayComparison  = this.comparison(tmallPrePayFinishedCount, lastTPrePayFinishedCount);
