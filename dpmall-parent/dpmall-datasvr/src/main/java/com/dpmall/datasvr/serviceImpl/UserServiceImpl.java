...
public class UserServiceImpl implements IUserService {
	...
	private LoginResModel entityToLoginModel(AppUserEntity entity) {
		...
		if (entity != null){
			BeanUtils.copyProperties(entity,model);
			model.appGroupModel = groupEntityToMoel(entity.appGroupEntity);
		}
		return model;
	}
	...
	public LoginResModel login(String username, String passwd) {
		...
		result.storeName = result.cnName;
		if (result.roleCode.equals("dailishang")) {//经销商
			result.roleCode = "agency";//同步以前的旧数据
			result.agencyId = entity.userId;
		} else if (result.roleCode.equals("mendian")) {//门店
			result.roleCode = "clerk";//同步以前的旧数据
			result.storeId = entity.userId;
			result.appGroupModel = null;//门店不需要分组
		} else {//其他类型
//			AppUserEntity infoEntity = userDao.getOrderUserInfo(String.valueOf(result.id));
			result.roleName = result.cnName;
			result.groupCode = result.username;
			result.groupName = result.cnName;
			result.storeName = "";
